    ...
    def initialize_logger(self, folder):
        if self.nesting:
            ...
            for i in self.nesting_list:
                self.val_meters['top_1_{}'.format(i)] = torchmetrics.Accuracy(task="multiclass", num_classes=100, top_k=1).to(self.gpu)

            for i in self.nesting_list:
                self.val_meters['top_5_{}'.format(i)] = torchmetrics.Accuracy(task="multiclass", num_classes=100, top_k=5).to(self.gpu)
